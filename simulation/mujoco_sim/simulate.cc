...
void MakeJointSection(mj::Simulate* sim, int oldstate) {
  mjuiDef defJoint[] = {
    {mjITEM_SECTION, "Configuration", oldstate, nullptr, "AJ"},
    {mjITEM_END}
  };
  mjuiDef defSlider[] = {
  ...
  mjuiDef defText[] = {
      {mjITEM_EDITNUM, "Text", 1, nullptr, ""}, {mjITEM_END}
  };
  ...
  for (int i=0; i < sim->jnt_type_.size() && itemcnt<mjMAXUIITEM; i++) {
    ...
    defSlider[0].state = 1;
    mju::strcpy_arr(defText[0].other, "1");
    for (int i = 0; i < 7; i++) {
      defText[0].pdata = &sim->d_->qpos[i];
      switch (i) {
          case 0:
              mju::strcpy_arr(defText[0].name, "x");
              break;
          case 1:
              mju::strcpy_arr(defText[0].name, "y");
              break;
          case 2:
              mju::strcpy_arr(defText[0].name, "z");
              break;
          case 3:
              mju::strcpy_arr(defText[0].name, "qw");
              break;
          case 4:
              mju::strcpy_arr(defText[0].name, "qx");
              break;
          case 5:
              mju::strcpy_arr(defText[0].name, "qy");
              break;
          case 6:
              mju::strcpy_arr(defText[0].name, "qz");
              break;
      }
      mjui_add(&sim->ui1, defText);
  }

}

// make velocity section of UI
void MakeVelocitySection(mj::Simulate* sim, int oldstate) {
  mjuiDef defVelocity[] = {
    {mjITEM_SECTION, "Velocities", oldstate, nullptr, "AC"},
    {mjITEM_END}
  };
  mjuiDef defText[] = {
      {mjITEM_EDITNUM, "Text", 1, nullptr, "T"}, {mjITEM_END}
  };

  // add section
  mjui_add(&sim->ui1, defVelocity);
    defText[0].state = 1;
    mju::strcpy_arr(defText[0].other, "1");

  // add controls, exit if UI limit reached (Clear button already added)
  int itemcnt = 1;
  for (int i = 0; i < sim->m_->nv && itemcnt<mjMAXUIITEM; i++) {

    // set data and name
    defText[0].pdata = &sim->d_->qvel[i];

      if (i - 6 <= sim->jnt_type_.size() && i > 5 && !sim->jnt_names_[i - 5].empty()) {
          mju::strcpy_arr(defText[0].name, sim->jnt_names_[i - 5].c_str());
      } else if(i <= 5) {
          switch (i) {
              case 0:
                  mju::strcpy_arr(defText[0].name, "x vel");
                  break;
              case 1:
                  mju::strcpy_arr(defText[0].name, "y vel");
                  break;
              case 2:
                  mju::strcpy_arr(defText[0].name, "z vel");
                  break;
              case 3:
                  mju::strcpy_arr(defText[0].name, "rot1 vel");
                  break;
              case 4:
                  mju::strcpy_arr(defText[0].name, "rot2 vel");
                  break;
              case 5:
                  mju::strcpy_arr(defText[0].name, "rot3 vel");
                  break;
              default:
                  mju::strcpy_arr(defText[0].name, "wrong");
                  break;
          }
      }
...
void MakeUiSections(mj::Simulate* sim, const mjModel* m, const mjData* d) {
  ...
  MakeJointSection(sim, oldstate1[SECT_JOINT]);
        MakeVelocitySection(sim, oldstate1[SECT_CONTROL]);
}
    ...
    // group section
      ...
      if (it->name[0]=='A' && it->name[1]=='c') {
        sim->ui1.nsect = SECT_CONTROL;
          MakeVelocitySection(sim, sim->ui1.sect[SECT_CONTROL].state);
        sim->ui1.nsect = NSECT1;
        UiModify(&sim->ui1, state, &sim->platform_ui->mjr_context());
      }
    }
...
void Simulate::Sync() {
  ...
  for (int i = 0; i < m_->nv; ++i) {
    if (qvel_[i] != qvel_prev_[i]) {
      d_->qvel[i] = qvel_[i];
    } else {
        qvel_[i] = d_->qvel[i];
    }
    if (qvel_prev_[i] != qvel_[i]) {
      pending_.ui_update_ctrl = true;
      qvel_prev_[i] = qvel_[i];
    }
  }
...
#define X(name)                                                  \
  ...
  // TODO: Make reset use the user defined configuration
  if (pending_.reset) {
    mj_resetData(m_, d_);
    mj_forward(m_, d_);
